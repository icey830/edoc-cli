...
var edoc = module.exports = function (data) {
    data = data || {};
    return through.obj(function (file, enc, cb) {
        var cwd = file.cwd;
        loadConfig(cwd, function (conf) {
            edoc.build(cwd, conf ? _extends(conf, data) : data);
            cb();
        });
    });
};

edoc.actions = actions;

edoc.init = actions.init;

edoc["new"] = actions["new"];

edoc.server = actions.server;

edoc.build = function (cwd, conf, opt) {
    opt = opt || {};
    ...
    execTemplate(destPath, tplPath, function (content) {
        ...
        if (opt.watch) {
            ...
            watch.watchTree(cwd, {
                ...
                var reg = /edoc.json$|edocfile.js$/gi;
