...
var json = require('../json_temp/edoc.config.json');
...
module.exports = function(cwd, conf) {
    var confFilePath = sysPath.join(cwd, 'edoc.config.json');
    ...
    inquirer.prompt([{
        ...
        if (fs.existsSync(confFilePath)) {
            ...
            inquirer.prompt([{
                type: 'confirm',
                message: '是否覆盖原有配置 edoc.config.json 文件?',
                name: 'ok'
            }]).then(function(res) {
                if (res.ok) {
                    fs.writeFileSync(confFilePath, data, 'UTF-8');
                    console.log('√ 覆盖原有 edoc.config.json 成功！'.green);
                }
            });
        } else {
            fs.writeFileSync(confFilePath, data, 'UTF-8');
            console.log('√ 生成 edoc.config.json 成功！'.green);
        }
    })
}
